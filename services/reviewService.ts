...
export async function getReviewRecord(year: number, month: number): Promise<ReviewRecord | null> {
...
export async function saveReviewRecord(record: ReviewRecord): Promise<void> {
  const records = await getReviewRecords();
  const filtered = records.filter((r) => !(r.year === record.year && r.month === record.month));
  filtered.push(record);
  await AsyncStorage.setItem(STORAGE_KEY, JSON.stringify(filtered));
}

export async function clearReviewRecord(year: number, month: number): Promise<void> {
  const records = await getReviewRecords();
  const filtered = records.filter((r) => !(r.year === year && r.month === month));
  await AsyncStorage.setItem(STORAGE_KEY, JSON.stringify(filtered));
}

export async function isMonthReviewed(year: number, month: number): Promise<boolean> {
