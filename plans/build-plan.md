...
- [ ] `npx create-expo-app picky-saver --template blank-typescript`
...
- [ ] Install dev/test packages:
  ```
  jest-expo jest @testing-library/react-native @types/react
  ```
...
**Implement the design system from the UI spec as code constants.**
...
- [ ] Create `constants/theme.ts` with:
  ...
  - `screenMargin: 20`
  - `touchTarget: { min: 48 }`
- [ ] Create `types/index.ts` with shared types: `MonthBatch` (year, month, count), `SwipeDirection` ('left' | 'right'), `SwipeSession` (kept, deleted, total)
- [ ] Set up dark/light mode support using React Native's `useColorScheme` + a `useTheme()` hook that returns the correct color set
...
**Build the permission layer — the first thing every user encounters.**
...
**Load photos from the library grouped by month, and build the date picker UI.**

- [ ] Create `services/photoService.ts`:
  - `getAvailableMonths()` — paginate through all assets via `MediaLibrary.getAssetsAsync({ sortBy: ['creationTime'], first: 500 })`, group by year/month, return `MonthBatch[]` sorted newest-first
  - `getPhotosForMonth(year, month)` — fetch using `createdAfter: new Date(year, month, 1).getTime()` and `createdBefore: new Date(year, month + 1, 0, 23, 59, 59).getTime()`, return Asset array
...
- [ ] Create `components/MonthTile.tsx`:
  - Surface-colored card (`#F2F0EC` / `#1E1E1E`), full width, 24px padding
...
- [ ] Write tests:
  - `__tests__/services/photoService.test.ts` — groups assets by month correctly, passes correct date params, handles empty library, handles pagination (hasNextPage)
...
**Build the core swipe experience — the heart of the app.**

- [ ] Create `components/SwipeOverlay.tsx`:
  - "KEEP" text + checkmark icon, green (`#4CAF7D`), positioned top-left of card
  - "DELETE" text + X icon, red (`#E05555`), positioned top-right of card
...
- [ ] Create `components/PhotoCard.tsx`:
  ...
  - Photo displayed via `<Image source={{ uri: asset.uri }}>`
...
- [ ] Build `app/swipe/[year]/[month].tsx`:
  ...
  - Displays current `PhotoCard`, preloads next photo URI
...
**Implement the "To Be Deleted" album management — the persistence layer for marked photos.**
...
**Show results after completing a swipe session.**
...
**Let users review, restore, or permanently delete marked photos.**

- [ ] Create `components/PhotoGrid.tsx`:
  - 3-column FlatList grid, 4px gaps between thumbnails
  - Each cell: square thumbnail via `<Image>`, tap opens full-screen preview
...
**Wire up the entry point — the first thing users see.**
...
- [ ] Handle stale state:
  - Photos deleted outside the app: when loading month photos or deletion album, filter out assets that no longer resolve
...
- [ ] Create `hooks/useTheme.ts` (if not done in step 3):
  - Uses `useColorScheme()` from react-native
  - Returns the correct color set from `theme.ts`
- [ ] Apply theme colors to every screen and component:
  - Backgrounds, text, cards, buttons, borders, overlays
  - Status bar adapts (light content on dark bg, dark content on light bg)
- [ ] Verify contrast ratios still meet 4.5:1 in dark mode
- [ ] Write test:
  - `__tests__/hooks/useTheme.test.ts` — returns dark colors when scheme is 'dark', light colors when 'light'

**Verify:** Toggle device appearance in Settings. App switches cleanly, all text readable, no stale background colors.
